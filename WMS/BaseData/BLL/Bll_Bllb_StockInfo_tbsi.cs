...
LEFT JOIN MdcdatMaterial MDC ON MDC.MATERIALCODE=T.MATERIALCODE
 ...
 GROUP BY T.MaterialCode,S.Storage_SN,R.Storage_Name,T.Status_Flag,MDC.SPEC) B WHERE B.QTY>0", StockTime, strWhere1, strWhere2, strWhere3);
        ...
        /// <returns></returns>
        public static bool OnLineSfcNoMove(string sfcno,string SerialNumber)
        {
            string strSql = string.Format(@" Update T_Bllb_StockInfo_tbsi set  SfcNo='{1}',TBS_ID=''  WHERE SerialNumber='{0}'", SerialNumber,sfcno);
