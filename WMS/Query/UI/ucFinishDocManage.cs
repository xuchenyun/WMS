...
namespace Query.UI
{
    public partial class ucFinishDocManage : UserControl
    {
        ...
        {
            Form_CreateFinishDocEdit f = new Form_CreateFinishDocEdit();
        ...
        {
            dtType = BLL_Bllb_StorageDoc_tbsd.QueryDocType("where TYPE_CODE in('18', '19')");
...
SqlInput.ChangeNullToString(dgv_StorageMaterial.Rows[dgv_StorageMaterial.CurrentCell.RowIndex].Cells[MaterialCode.Name].Value));
        ...
        private void tbl_edit_Click(object sender, EventArgs e)
        {
            Form_ShowDocDetail f = new Form_ShowDocDetail();
            f.list_dgvr = new List<DataGridViewRow>();
            int isSelectedCount = 0;
            foreach (DataGridViewRow dgvr in dgv_StorageDoc.Rows)
            {
                if (dgvr.Cells[Chk.Name].EditedFormattedValue != null && Convert.ToBoolean(dgvr.Cells[Chk.Name].EditedFormattedValue) == true)
                {
                    isSelectedCount++;
                    f.list_dgvr.Add(dgvr);
                }
            }
