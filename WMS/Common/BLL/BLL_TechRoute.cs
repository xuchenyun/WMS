...
LEFT JOIN T_Bllb_techGroupRelation_tbtgr B ON T.TBTG_ID=B.F_TBTG_ID
 LEFT JOIN T_Bllb_technologyGroup_tbtg TB ON B.TBTG_ID=TB.TBTG_ID
  ...
  LEFT JOIN T_Bllb_group_tbg TBG ON TBG.TBG_ID=TB.TBG_ID WHERE T.TBPS_ID='{0}'", TBPS_ID);
        ...
        /// <param name="TBS_ID"></param>
        /// <returns></returns>
        public static string CheckRouteNoControl(string serial_Number, string TBS_ID, ref string TBTG_ID)
        {
            ...
            if (dt_Product.Rows.Count > 0)//已投产的产品（在制品）
            {
                ...
                //流程校验
                msg = CheckRouteNoControl(TBT_ID, F_TBTG_ID, TBS_ID, is_error, ref TBTG_ID);
                if (msg != "OK")
                {
                    return msg;
                }
        ...
        /// <returns></returns>
        public static string CheckRouteNoControl(string TBT_ID, string F_TBTG_ID, string TBS_ID, bool errorFlag, ref string next_TBTG_ID)
        {
            ...
            if (errorFlag == true)
            {
                DataRow[] drs = dt_Route.Select(string.Format("F_TBTG_ID='{0}'", F_TBTG_ID));
                //判断当前要过的工艺工序是否是正确
            ...
            {
                DataRow[] drs = dt_Route.Select();//获取非维修工序
        ...
        /// <param name="errorFlag"></param>
        /// <returns></returns>
        public static string CheckRoute(string TBT_ID, string F_TBTG_ID, string TBS_ID, bool errorFlag, ref string NOW_TBTG_ID, ref string wip_tbtg_id)
        {
            //获取当前产品的下一个工序（可能有多个，0正常、1维修）
            string GROUP_TYPE = errorFlag == true ? "1" : "0";
            DataTable dt_Route = GetListofRoute(TBT_ID);
            ...
            if (errorFlag == true)
            {
                ...
                foreach (DataRow dr in drs)
                {
                    ...
                    {
                        NOW_TBTG_ID = dr["TBTG_ID"].ToString();
                        return "OK";
                    }
                }
                if (next_GroupName == string.Empty)
            ...
            {
                DataRow[] drs = dt_Route.Select(string.Format("GROUP_TYPE='{0}'", GROUP_TYPE));//获取非维修工序
                next_GroupName += GetNextGroup(F_TBTG_ID, str_tbtg_id, drs, ref NOW_TBTG_ID);
