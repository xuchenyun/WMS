...
WHERE T.SERIAL_NUMBER='{0}' AND T.LAST_FLAG='Y'", productSN);
        ...
        private static bool UpdateOverTech(Model_Bllb_productInfo_tbpi obj)
        {
            ...
            string strSql = string.Format(@"UPDATE T_Bllb_productInfo_tbpi SET
                                        OVER_FLAG='{1}',ONCE_OVER_FLAG='Y',SERIAL_NUMBER='{2}',OUTPUT_TIME=GETDATE()
                                        where
                                        SERIAL_NUMBER='{0}'
                                        ",
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public static bool UpdateErrorFlag(string SERIAL_NUMBER)
        {
            string strSql = string.Format(@"UPDATE T_Bllb_productInfo_tbpi SET
                                        ERROR_FLAG='Y'
                                        where
        ...
        /// 设置产品良品状态或者不良品状态
        /// </summary>
        /// <param name="TBPS_ID">产品ID</param>
        /// <param name="ERROR_FLAG">不良品/良品（Y/N）</param>
        /// <returns></returns>
        public static bool UpdateErrorFlag(string TBPS_ID, string ERROR_FLAG)
        {
            string strSql = string.Format(@"UPDATE T_Bllb_productInfo_tbpi SET
                                        ERROR_FLAG='{1}'
                                        where
        ...
        public static bool SetNextGroup(String TBPS_ID,string RE_TBTG_ID)
        {
            string strSql = string.Format(@"UPDATE T_Bllb_productStatus_tbps SET
                                        ...
                                        where
                                        TBPS_ID='{0}'
										UPDATE dbo.T_Bllb_productInfo_tbpi SET ERROR_FLAG='N' WHERE TBPS_ID='{0}'
                                        ",
                                    TBPS_ID, RE_TBTG_ID);
            return CIT.Wcf.Utils.NMS.ExecTransql(PubUtils.uContext, strSql);
        }
        ...
        /// <param name="RE_TBTG_ID"></param>
        /// <returns></returns>
        public static bool SetNextGroupByPanelCode(String PanelCode, string RE_TBTG_ID)
        {
            string strSql = string.Format(@"UPDATE T_Bllb_productStatus_tbps SET
                                        RE_TBTG_ID='{1}',WIP_TBTG_ID='{1}'
                                        WHERE TBPS_ID IN (SELECT TBPS_ID FROM dbo.T_Bllb_productInfo_tbpi
                                        WHERE PanelCode='{0}')
	UPDATE dbo.T_Bllb_productInfo_tbpi SET ERROR_FLAG='N' WHERE TBPS_ID IN (SELECT TBPS_ID FROM dbo.T_Bllb_productInfo_tbpi
                                        WHERE PanelCode='{0}')
                                        ",
                                    PanelCode, RE_TBTG_ID);
            return CIT.Wcf.Utils.NMS.ExecTransql(PubUtils.uContext, strSql);
        }
        #endregion
        #region 设置检验的重新投入工序
   ...
   where  TBPS_ID in (select T.TBPS_ID from T_Bllb_sampleProduct_tbap T WHERE T.DOC_NO='{0}')",
        ...
        private static bool InsertProductPass(Model_Bllb_productPass_tbpp obj)
        {
            string strSql = string.Format(@"INSERT INTO T_Bllb_productPass_tbpp(
                                        ...
                                        obj.STATE_FLAG,
                                        obj.PLCode, 
                                        PubUtils.uContext.UserID,obj.TBS_ID
                                        );
            return CIT.Wcf.Utils.NMS.ExecTransql(PubUtils.uContext, strSql);
