    ...
    public class T_Bllb_productInfo_tbpi_DAL
    {
        ...
        public DataTable GetBasicData(string strWhere)
        {
            string strSql = string.Format(@" SELECT Tbpi.AGING_START_TIME,Tbpi.AGING_END_TIME,Tbpi.INOUT_STATUS,Product.WoCode,Product.SfcNo,Tbps.PLCode,ProLine.PLName,PanelCode,
                             twn.Version,Tbpi.OVER_FLAG,Tbpi.ERROR_FLAG,Tbpi.REPAIR_FLAG,Tbpi.SCRAP_FLAG,Product.Product,Product.ProductName,G.Group_Name,Two.CONTAINER_SN_2,One.CONTAINER_SN_1 
                                ...
                                LEFT JOIN T_Bllb_packageTwo_tbpt AS Two ON One.CONTAINER_SN_1=Two.CONTAINER_SN_1
								LEFT JOIN dbo.T_Work_Number TWN ON TWN.WoCode=Product.WoCode {0}", strWhere);
            return NMS.QueryDataTable(PubUtils.uContext, strSql);
        }
        /// <summary>
        /// 最后过本工序的明细信息
...
LEFT JOIN dbo.T_Work_Number W ON B.WoCode=W.WoCode
 where SERIAL_NUMBER = '{0}' AND LAST_FLAG = 'Y' ", tbpi.SERIAL_NUMBER, tbpi.ProductCode, tbpi.Version);
        ...
        public DataTable Repair(string strWhere)
        {
            string strSql = string.Format(@"SELECT  a.SERIAL_NUMBER ,
        a.SfcNo ,
        c.Line ,
        CASE WHEN d.REPAIR_MAN IS NULL THEN '未维修'
             ELSE '已维修'
        END AS 'REPAIR' ,
        CASE a.REPAIR_FLAG
          WHEN 'Y' THEN '是'
        ...
        e.TBEC_CODE ,
        f.TBER_NAME
FROM    T_Bllb_productError_tbpe d
        LEFT JOIN T_Bllb_productInfo_tbpi a ON a.TBPS_ID = d.TBPS_ID
        LEFT JOIN T_Bllb_productStatus_tbps b ON d.TBPS_ID = b.TBPS_ID
        LEFT JOIN SfcDatProduct c ON a.SfcNo = c.SfcNo
        LEFT JOIN dbo.T_Bllb_group_tbg g ON d.TBG_ID = g.TBG_ID
        LEFT JOIN dbo.T_Bllb_errorCode_tbec e ON e.TBEC_ID = d.TBEC_ID
        LEFT JOIN dbo.T_Bllb_errorReason_tber f ON f.TBER_ID = d.TBER_ID
        LEFT JOIN SysDatUser u ON u.UserID = d.ERROR_MAN {0}", strWhere);
            return NMS.QueryDataTable(PubUtils.uContext, strSql);
        }
    }
}
